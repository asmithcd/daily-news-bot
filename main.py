...
# API key for NewsAPI.org (set this as an environment variable for safety)
NEWSAPI_KEY = os.environ.get("NEWSAPI_KEY")

# Gmail credentials (use an app password if 2‑factor authentication is enabled)
...
# Time window: search for articles published in the last 24 hours
HOURS_LOOKBACK = 24

# Number of articles to retrieve per industry.  Adjust this value to
# retrieve more or fewer stories for each industry.
ARTICLES_PER_INDUSTRY = 3

# Define industry queries.  Each entry contains a search query and an
# optional comma‑separated list of trusted domains.  You can refine the
# queries or add/remove domains to improve relevance.  The keys
# correspond to the industry names that will appear as headings in the
# digest (they will be upper‑cased automatically).
QUERIES: Dict[str, Dict[str, str]] = {
    "APPLIANCES": {
        # Search terms related to appliance recycling, efficiency and industry
        "q": "(appliance OR appliances) AND (industry OR recycling OR regulation)",
        # Specify domains to keep results on major outlets and press releases
...
def get_recent_timestamp(hours_back: int) -> str:
    """Return an ISO‑8601 timestamp hours_back hours ago in UTC."""
    from_dt = datetime.datetime.utcnow() - datetime.timedelta(hours=hours_back)
    return from_dt.strftime("%Y-%m-%dT%H:%M:%SZ")


def fetch_articles(q: str, domains: str | None = None) -> List[Dict]:
    """Fetch recent articles matching the given query and domains from NewsAPI.
...
def summarize(text: str, max_sentences: int = 3) -> str:
    """A simple summarizer that returns the first max_sentences sentences."""
    if not text:
        return ""
...
def build_digest() -> str:
    """Compile the news digest as an HTML string with headings and hyperlinks."""
    lines: List[str] = []
    for industry in sorted(QUERIES.keys()):
        params = QUERIES[industry]
        articles = fetch_articles(params["q"], params.get("domains"))
        if not articles:
            # Skip industries with no relevant articles in the last 24 hours
            continue
        # Add industry heading (underlined and bold)
        ...
        for art in articles:
            ...
            # Create hyperlink for the title
            article_html = f'<p><a href="{url}" target="_blank">{title}</a>: {summary}</p>'
            lines.append(article_html)
        # Add a blank line between industries
        lines.append("<br/>")
    return "\n".join(lines)


def send_email(html_body: str) -> None:
    """Send the email digest using Gmail via SMTP."""
